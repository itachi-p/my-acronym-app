...
- **機械判定（`lib/candidate-filter.ts` `runMachineChecks`、INSERT前・
  LLM不要。判定順序はA1→D1→D2→S1→D5）**:
  - A1 短すぎる略語: 2文字以下。許可リスト例外:
    `BS`/`PL`/`LP`/`EV`/`PH`（decisions.md 24章）
  - D1 名称不確実: `full_spelling`に「または」「/」を含む複数候補
    併記。許可リスト例外: `PPAP`
  - D2 自己言及: `full_spelling`が`acronym`をそのまま含む。
    許可リスト例外: `MIT License`
  - S1 収録範囲外（2026-08-30追加）: Groqが返す`scope_fit`が`"out"`。
    `scope_fit`未指定（フィールド欠落等）はfail-openで通過させる。
    文字数(A1)に関わらず何文字の略語にも適用される
...
- **Groq側の判定（プロンプト、`app/api/acronym/route.ts`
  ...
  通用するもののみ、IT分野は内輪用語を除外（YAGNI/DRY等の
  他分野応用可能な原則、およびVibeCoding関連用語は対象）、
  廃止済み機関・歴史用語は対象に含める、分野タグを1つも決められない
  場合は返さない、をプロンプトに明記。Groqがこれに反する候補を
  返しても機械判定（A1/D1/D2/S1/D5）は必ず通す
- **収録範囲（スコープ、2026-08-30追加）**: 主軸はビジネス・経営・
  金融・投資・会計。周辺として政治・行政・国際機関・軍事・
  安全保障・ITはビジネス/時事の文脈に限り対象。AIを使った
  アプリ開発（VibeCoding）関連用語・分野横断的な開発原則
  （YAGNI/DRY等）も対象。医療・学術は社会問題と結びつく限り対象
  （例: mRNA、HIV）。専門医学用語・専門心理学用語・コーデックや
  通信規格等の技術仕様・一企業内部の製品名は対象外。各候補について
  Groqに`scope_fit`（"in"/"out"）と`scope_reason`を返させ、
  `"out"`は機械判定S1で棄却する。プロンプト指示だけでは遵守が
  保証されない（A1と同じ理由）ため、コード側にも判定を置く二重
  構造。詳細はdecisions.md 27章参照
...
- 2文字目以降での絞り込み・あいまい検索・ページネーションは
  未実装（YAGNI、TODO.md参照）
- 2026-08-27追記: 案内文の表記を「アクロニム」から「略語」へ統一
  （アプリ内の他のユーザー向け文言との整合、decisions.md 26章(b)）。
  総登録件数の表示も`<hr>`区切り＋強調スタイルに変更
...
- **画像**: `public/help/`配下に配置し、マークダウンから
  ...
  （横幅はコンテナに収まる）・`mx-auto`（中央寄せ）を指定。
  375〜1280pxで横スクロールが発生しないことをPlaywrightで確認済み
  （作業用の一時スクリプト、リポジトリには残していない）
- **2026-08-26追記（レターボックス解消）**: 当初`w-full`＋
  `object-contain`を併用していたため、`w-full`がbox幅をラッパー
  実効幅（`max-w-lg`由来の480px）まで強制し、`max-h-[70vh]`との
  高さ制約と組み合わさって、縦長画像（346×750相当）が実寸より
  大幅に縮小され左右に大きな空白（レターボックス、box 480×491px
  に対し絵226×491px、空白254px＝枠面積の53%）が生じていた。
  `w-full`を削除し、preflight由来の`height:auto`とmax-heightの
  制約から幅を画像の縦横比で逆算させることで、枠が絵の実寸に
  密着するようにした。boxと絵の寸法差がなくなったため
  `object-contain`は不要になり削除した（他箇所での使用なし。
  decisions.md参照）
- **2026-08-27追記（PC向けグリッド化）**: `max-w-lg`固定の1カラム
  構成だったコンテナを、1024px以上のみ`max-w-4xl`に拡張し、
  `.help-article`（`app/globals.css`）を3カラムのCSS Gridにした。
  画像単体の段落だけがグリッドセルに収まり、見出し・本文・リストは
...
- **ナビゲーション**: トップ画面（`app/page.tsx`）の最下部に
  「ヘルプ」導線を設置（2026-08-27: 目立たないテキストリンクから、
  `/help`側の導線と統一感のあるボーダー付きピルボタンに変更）。
  `/help`側は右上に既存パターンと同じ「🔍 検索画面へ」ピルリンクを
  設置
